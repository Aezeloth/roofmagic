...
// Checks user's private KV first, then app-level public KV — anonymous allowed for public projects
router.get('/api/projects/get', async ({request, user}) => {
    try {
        ...
        // Try private KV first (only if authenticated)
        const userPuter = user?.puter;
        if(userPuter) {
            const { uuid: userId } = await getUserInfo(userPuter);
            if(userId) {
        ...
        // Fall back to public KV (accessible to everyone including guests)
        if(!project) {
            project = await me.puter.kv.get(`${PUBLIC_PROJECT_PREFIX}${id}`);
        }
...
// Moves project from user's private KV → app-level public KV
// Public key is scoped to ownerId to prevent collisions between users
router.post('/api/projects/share', async ({request, user}) => {
    try {
        ...
        // Key is scoped to userId_projectId to prevent cross-user collisions
        const publicKey = `${PUBLIC_PROJECT_PREFIX}${userId}_${id}`;
...
// Moves project from app-level public KV → user's private KV (owner only)
router.post('/api/projects/unshare', async ({request, user}) => {
    try {
        ...
        await userPuter.kv.set(`${PROJECT_PREFIX}${id}`, privatePayload);
